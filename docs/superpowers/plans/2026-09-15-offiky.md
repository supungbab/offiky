...
10개 작업을 모두 구현했다. 테스트 36건 통과, 맥 한 대에서 동작 확인.

**이 계획과 실제 구현이 다른 곳은 아래와 같다. 현재 설계는 스펙 문서가 기준이다.**

| 항목 | 계획 | 실제 | 이유 |
|---|---|---|---|
| 클릭 통과 | 투명 픽셀은 통과한다고 가정 | 성립하지 않음 | Task 1 실기 확인. 오버레이를 표시 전용으로 두고 드래그용 윈도우를 따로 둔다 |
| 표시 크기 | 16pt | 32pt | 이름을 항상 표시하게 되어 16pt 로 줄여 얻는 것이 없어졌고, 걸어 다니는 16pt 드래그 표적이 잡기 어려웠다 |
| 스프라이트 인코딩 | 팔레트 인덱스 1자/픽셀 | `RRGGBB` 6자/픽셀 | 색 수를 16가지로 제한하지 않기 위해 |
| 기본 캐릭터 | 사람 형태, 머리×옷 64조합 | 어몽어스 크루메이트, 몸통 12색 | 요청 |
| 이름 표시 | 커서 근접 시에만 | 항상 | 몸통 색 12가지로는 식별이 안 된다 |
| 외형 결정 | `id` 해시 | `installID` 해시 | `id` 는 실행마다 바뀌어 껐다 켤 때마다 색이 달라졌다 |
| 단축키 | `KeyboardShortcuts` SPM, `⌃⌥Space` | Carbon 직접 등록, `⌥Space` | SPM 추가는 Xcode GUI 가 필요해 진행이 막힌다. `⌃⌥Space` 는 macOS 입력 소스 전환이 선점한다 |
| 채팅 입력 | 화면 중앙 패널 | 화면 가운데 아래, 포커스 복귀 | 요청 |
| 점프 | 정점 24pt, 중력 2400 | 정점 48pt, 중력 1100, 등가속도 적분 | 속도만 더하는 적분은 12fps 에서 정점이 절반으로 줄어든다 |

**구현하지 않은 것**

- 단축키 재지정 UI — `KeyboardShortcuts` 패키지를 Xcode 에서 추가하면 붙일 수 있다
- 델타 스냅샷 (스펙 6.4) — 측정 전에는 적용하지 않는다

**맥 한 대로는 확인할 수 없어 남은 것**

동료 캐릭터 표시, 좌표 보간, 호스트 선출과 교체, 채팅 전파, 프로필 전파.
Bonjour 광고 자체는 `dns-sd -B _offiky._tcp local.` 로 확인했다.
