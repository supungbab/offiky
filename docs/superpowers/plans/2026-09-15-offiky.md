...
- 스프라이트: 16×16 픽셀, 화면 표시 크기 32×32 포인트, `SKTexture.filteringMode = .nearest`
...
- 벽: 내 캐릭터는 `[16, 띠 길이 − 16]` 안에서만 이동한다 (스프라이트 폭 32의 절반)
- 중력: 2400 pt/s². 점프 정점 48pt
...
**1·2번이 통과하면** 가정이 맞다. 그대로 진행한다.

**1번 또는 2번이 실패하면** (투명 영역이 클릭을 가로채면) 스펙 11절의 대비책으로 전환한다. 아래처럼 바꾸고 Step 7~8 을 다시 수행한다.

```swift
window.ignoresMouseEvents = true
```

이 경우 Task 5 의 드래그는 오버레이가 아니라 **내 캐릭터를 따라다니는 32×32 짜리 별도 윈도우**에서 받는다. Task 5 진입 전에 그 윈도우를 만드는 단계를 추가한다. 동료 캐릭터는 어차피 클릭을 받지 않으므로 추가되는 윈도우는 1개다.

**3번이 실패하면**(사각형 위 클릭도 통과하면) `window.ignoresMouseEvents` 가 `true` 로 남아 있는지 확인한다.
...
- Produces:
  ...
  - `let protocolVersion = 1`, `let spriteDisplaySize: CGFloat = 32`
...
struct FloorStripTests {
    ...
    @Test func 왼쪽_끝은_첫_화면의_왼쪽이다() throws {
        ...
        #expect(p.point.x == 0)
        #expect(p.point.y == 16)          // 스프라이트 높이의 절반
    }
    ...
    @Test func 완전히_벗어나면_표시하지_않는다() {
        #expect(single.place(x: 1017, y: 0) == nil)   // 1017 - 16 > 1000
        #expect(single.place(x: -17, y: 0) == nil)    // -17 + 16 < 0
    }

    @Test func 높이는_화면_안으로_제한된다() throws {
        // 두 번째 화면 높이 600, 스프라이트 32 -> 최대 568
        let p = try #require(dual.place(x: 1200, y: 5000))
        #expect(p.point.y == 16 + 568)
    }

    @Test func 벽은_스프라이트_중심_기준이다() {
        #expect(single.clampToWall(-50) == 16)
        #expect(single.clampToWall(5000) == 984)      // 1000 - 16
...
final class CharacterNode: SKNode {
    ...
    private func simulate(dt: TimeInterval, now: TimeInterval, strip: FloorStrip) {
        ...
        if now >= nextJumpAt {
            ...
            verticalSpeed = (2 * 2400 * 48).squareRoot()   // 정점 48pt
            return
        }
...
**Task 1 에서 클릭 통과가 성립하지 않는 것으로 확인됐다.** 오버레이는 `ignoresMouseEvents = true` 인 표시 전용이므로 씬은 마우스 이벤트를 받지 못한다. 대신 **내 캐릭터를 따라다니는 32×32 포인트 윈도우**를 만들고 거기서 이벤트를 받는다.
