...
`offiky` 타깃 블록은 `PRODUCT_BUNDLE_IDENTIFIER = com.offiky;` 가 있는 블록이다. 기존 `GENERATE_INFOPLIST_FILE = YES;` 줄을 위 네 줄로 교체한다.
...
`CharacterScene` 에 추가한다. Task 1 Step 9 에서 대비책으로 전환했다면 이 단계 대신 내 캐릭터를 따라다니는 32×32 윈도우를 만들고 거기서 같은 이벤트를 받는다.

```swift
extension CharacterScene {
    /// 이 씬이 담당하는 화면의 띠 시작 오프셋. rebuild() 에서 설정한다.
    var stripOffset: CGFloat {
        get { userData?["stripOffset"] as? CGFloat ?? 0 }
        set {
            if userData == nil { userData = NSMutableDictionary() }
            userData?["stripOffset"] = newValue
        }
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        guard me.parent === self,
              abs(p.x - me.position.x) <= spriteDisplaySize / 2,
              abs(p.y - me.position.y) <= spriteDisplaySize / 2
        else { return }
        World.shared.beginDrag()
        World.shared.updateDrag(toStripX: stripOffset + p.x, sceneY: p.y)
    }
...
`OverlayController.rebuild()` 에서 씬을 만든 뒤 누적 폭을 넣어준다.

```swift
var accumulated: CGFloat = 0
for (index, scene) in scenes.enumerated() {
    scene.stripOffset = accumulated
    accumulated += strip.frames[index].width
}
```
...
extension World {
    ...
    /// 가로는 벽으로 제한하고, 세로는 바닥 아래로 내려가지 않게 한다.
    func updateDrag(toStripX stripX: CGFloat, sceneY: CGFloat) {
        me.x = strip.clampToWall(stripX)
        me.y = max(0, sceneY - spriteDisplaySize / 2)
    }
