...
**Offi**ce + Luc**ky**. 사무실에 행운 한 마리. 친한 동료들과 놀려고 만들었고,
읽을 때는 오피키다.
...
| **연결** | 바깥 서버는 없다. **먼저 맡은 사람이 계속 호스트**고, 나머지는 호스트에게만 연결한다 |
| **호스트 교체** | 뽑는 절차가 없다 — 호스트는 광고에 맡고 있다고 표시하고, 그게 보이면 모두 그 사람에게 붙는다. 새로 들어온 사람이 자리를 뺏지 않는다. 호스트가 사라지면 남은 사람 중 id 가 가장 작은 사람이 이어받고, 둘이 동시에 맡았다고 하면 같은 규칙으로 갈린다 |
